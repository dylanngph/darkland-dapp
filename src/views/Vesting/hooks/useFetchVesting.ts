...
export const useFetchVestingTGE = (): vestingPropsType => {
  ...
  const fetchVestingTGE = useCallback(async () => {
    ...
    try {
      ...
      const [getAllocation , getReleased , getTGERelease , vestable , getCliffDuration] = await multicallv2(vestingAbi, calls)
    ...
    try {
      const calls = () => {
        ...
        for (let i = 0; i < getAllStage; i++) {
          stages.push({
            address: getAddress(VestingTGE.contractAddress.privateSale),
            name: 'listStage',
            params: [i],
          })
        }
      ...
      const listStage = await multicallv2(vestingAbi, calls())
      ...
      const isClaimed = await multicallv2(vestingAbi, callsClaimed())
