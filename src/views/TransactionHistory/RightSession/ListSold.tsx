...
import { tabTransactionHistory } from 'views/MarketPlace/constant'

const ListSold = ({ listHeroes }) => {
  const { t } = useTranslation()
  const [tab, setTab] = useState(tabTransactionHistory[0].label)
  ...
  const renderTabHeader = () => (
    ...
    >
      <TabList>
        {tabTransactionHistory.map((tabItem) => {
          return (
            <TabCustom
              aria-hidden="true"
              key={tabItem.label}
  ...
  return (
    <div>
      <Block>
        <h1 className="text-xl font-bold">Recently sold</h1>
      </Block>
      <Card>
