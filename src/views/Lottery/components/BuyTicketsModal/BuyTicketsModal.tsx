...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({onDismiss}) => {
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      ...
      if (limitedMaxPurchase.lt(maxNumberTicketsPerBuyOrClaim)) {
        ...
        const {ticketsBoughtWithDiscount: secondTicketDiscountBuy} = getMaxTicketBuyWithDiscount(maxPlusDiscountTickets)
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient HTD balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInCake.times(new BigNumber(ticketsToBuy))) : '0.00'} HTD`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInCake && getFullDisplayBalance(priceTicketInCake.times(ticketsToBuy || 0))} HTD
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
