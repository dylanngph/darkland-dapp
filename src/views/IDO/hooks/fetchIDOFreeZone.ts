...
import idoABI from 'config/abi/idoFreezone.json'
...
export const fetchUserIsLockFree = async (account: string, idoAddress: Address, idoId: number) => {
  ...
  try {
    ...
    const [[isPaid], [isClaim], timeClaimIDO, startTime, endTime, maxHardCap, nowCap, nowTotalUser] = await multicallv2(
      idoABI,
      calls,
    )
    result = {
      ...
      maxHardCap: Number(new BigNumber(maxHardCap).toJSON()) / 10 ** idoInfo.tokenPrice.decimals,
      nowCap: Number(new BigNumber(nowCap).toJSON()) / 10 ** idoInfo.tokenPrice.decimals,
      nowTotalUser: Number(new BigNumber(nowTotalUser).toJSON()),
